...
The four metric cards (CPU, RAM, temperature, storage) respond visually to threshold crossings:
...
The temperature card's large number renders as a gradient that shifts along a thermal spectrum:

| Range | Colour |
|-------|--------|
| Below ~45 C | Accent blue (cool) |
| ~45-60 C | Blends toward amber (warm) |
| Above ~70 C | Red (hot) |

The gradient position animates smoothly over 1.5 seconds using a registered CSS `@property`. The card also has a subtle ambient glow that shifts through the same spectrum. In browsers without `@property` support, the colour still changes but snaps instead of animating.
