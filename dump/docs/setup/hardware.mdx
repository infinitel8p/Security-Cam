...
:::info
It is assumed that the Raspberry Pi Zero 2 W is already set up and running (Optional - set up VNC if you have a hard time working in a headless setup). If you need help setting up the Raspberry Pi Zero 2 W, please refer to the official [Raspberry Pi Documentation](https://www.raspberrypi.org/documentation/).
:::
...
Find `camera_auto_detect=1` and modify it to `camera_auto_detect=0`.
...
```bash
python3 -c "import cv2; cap=cv2.VideoCapture(0, cv2.CAP_V4L2); cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640); cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480); cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*'MJPG')); ret,frame=cap.read(); cv2.imwrite('opencv_test.jpg', frame) if ret else print('Error: Could not read frame'); cap.release()"
```
...
5. #### Enable automatic bus configuration:
   ...
   ```bash
   echo -e "[Unit]\nDescription=Load DS3231 RTC on boot\nAfter=multi-user.target\n\n[Service]\nType=oneshot\nExecStart=/bin/sh -c 'echo ds3231 0x68 > /sys/class/i2c-adapter/i2c-1/new_device'\nRemainAfterExit=yes\n\n[Install]\nWantedBy=multi-user.target" | sudo tee /etc/systemd/system/ds3231.service && sudo systemctl enable ds3231.service
   ```
...
Continue to [wifi.mdx](./wifi) to set up the Raspberry Pi as an access point and connect to it.
