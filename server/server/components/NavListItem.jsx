...
function NavListItem({ title, link, state, makeActive }) {
	return (
    <button className={`rounded-2xl border-l-[5px] bg-[var(--color-secondary)] ${state == title ? "border-[var(--color-tertiary)]" : "border-[var(--color-secondary)]"}`} onClick={() => makeActive(title)} >
      <Link
        href={link} 
