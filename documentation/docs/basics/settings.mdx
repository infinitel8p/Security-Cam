...
A default password is generated on first startup and logged to the Pi console. After enabling auth, change it using the **Change password** section.
...
:::tip Locked out?
If you forget the password, edit the settings file via SSH to disable auth:
```bash
...
```
Then restart the service. A new default password will be generated and logged to the console.
:::
