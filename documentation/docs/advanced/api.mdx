...
When authentication is enabled (see [Settings - Security](/docs/basics/settings#security)), the dashboard requires a password to log in. For programmatic access (scripts, curl), use the API token shown in Settings.
...
| **[System](#system-endpoints)** | `/system/alert`, `/system_info`, `/system/restart`, `/system/reboot`, `/system/update/check`, `/captive-portal` |
| **[Media](#snapshot-endpoint)** | `/snapshot`, `/thumbnail/<file>`, `/sprite/<file>` |
| **[Archive](#archive-badge-endpoint)** | `/archive/count` |
| **[Auth](#auth-endpoints)** | `/auth/status`, `/auth/validate`, `/auth/regenerate`, `/auth/set-password` |
