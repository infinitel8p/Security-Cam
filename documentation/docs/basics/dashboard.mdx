...
- **Night Vision Badge** - when using a NoIR camera with IR illumination, a purple **IR** badge appears on the live feed. The system detects the characteristic pink/magenta color cast produced by infrared light by periodically sampling a frame and analyzing its hue distribution. Detection thresholds are configurable in Settings under `NightVision`
...
- **System Health** - CPU temperature, load, RAM usage, storage, uptime, and throttle status. When metrics cross [warning or critical thresholds](./visual-guide#system-monitor-thresholds), a toast notification appears and an alert dot shows on the Dashboard nav icon. For detailed system stats with history charts and SD card info, see the [Stats page](./stats)
...
Configure the system without touching config files. The settings page has a sidebar with six sections: Appearance, Camera, Storage, Devices, Recording Sensors, and System. See the [Settings page guide](./settings) for full details on each section.
...
View system logs directly from the dashboard without needing SSH access. For common log patterns and debugging tips, see [Troubleshooting](./troubleshooting). The logs page has three tabs:
...
When a trigger sensor is enabled, it gates recording based on device presence - recording only starts if no tracked device is nearby. See [how it works](./sensors#how-it-works) in the sensors guide for the full flow. Manual recording is always allowed regardless of sensor state or presence.
