...
If you ran `setup_ap.sh`, both are already configured. To disable the captive portal, remove the `address=` lines from the dnsmasq config and the iptables PREROUTING rule, then restart dnsmasq.
