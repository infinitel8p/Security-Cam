"""KY-038 small sound sensor (standard sensitivity microphone).

Same circuit as the KY-037 but with a smaller, less sensitive
microphone element.  Better for detecting nearby sounds rather
than room-wide noise.

Wiring (3 pins - ignore AO):
  + (VCC) → 3V3 [pin 17]
  GND     → GND  [pin 14]
  DO      → GPIO [default pin 8]
...
log = logging.getLogger("sensor.sound_small")


class SoundSmallSensor(BaseSensor):
    name = "Sound (Small)"
    sensor_type = "sound_small"
    default_gpio = 8
    module = "KY-038"
    description = "Standard microphone - triggers on loud sounds (less sensitive than KY-037)"
    use_case = "Nearby sound detection (knocking, tapping, close-range noise)"
    icon = "zap"
    wiring = (
        {"pin": "+ (VCC)", "connect": "3V3 [pin 17]"},
        {"pin": "GND", "connect": "GND [pin 14]"},
        {"pin": "DO", "connect": "GPIO 8 [pin 24]"},
    )
    ...
    def start(self, on_trigger, on_release) -> None:
        ...
        self._pulse_times = []
        # KY-038 DO: normally high, pulses low on loud sound
        ...
        self._running = True
        log.info("Sound (small) sensor started on GPIO %d (pulse_count=%d, window=%ds)",
                 self.gpio, self._pulse_count, self._pulse_window)

    def _handle_pulse(self):
        if self._pulse_count <= 1:
            log.info("Sound detected (small mic)")
            if self._on_trigger:
                self._on_trigger()
            return
        ...
        log.debug("Sound pulse %d/%d (small)", count, self._pulse_count)
        if count >= self._pulse_count:
            log.info("Sound threshold reached (%d pulses in %ds, small mic)",
    ...
    def _handle_release(self):
        log.debug("Sound pulse ended (small)")
    ...
    def stop(self) -> None:
        ...
        if self._device:
            ...
            self._device = None
        self._running = False
        log.info("Sound (small) sensor stopped")
